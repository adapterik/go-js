...
func (d *htmlDocument) GetElementsByName(elementName string) *NodeList {
	res := d.Call("getElementsByName", elementName)
	if res == nil {
		return nil
	}
	return &NodeList{res, true}
}
...
func (s DOMString) String() string {
	return s.RawString
}

func stringerToInterface(s []fmt.Stringer) []interface{} {
	f := make([]interface{}, len(s))
	for i, v := range s {
		f[i] = v.String()
	}
...
func (d *htmlDocument) Write(s ...fmt.Stringer) {
	d.Call("write", stringerToInterface(s)...)
}

func (d *htmlDocument) Writeln(s ...fmt.Stringer) {
	d.Call("writeln", stringerToInterface(s)...)
}
...
func (w *window) HTMLDocument() HTMLDocument {
	rawDoc := w.Get("document")
	switch elementConstructor(rawDoc) {
	case js.Global.Get("HTMLDocument"):
		return &htmlDocument{&document{&BasicNode{rawDoc}}}
	default:
		panic("not an html document!")
	}
}
